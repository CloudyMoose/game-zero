...
function AnimationEditor:spawn(unit)
	if self.unit then self:unspawn() end
...
function AnimationEditor:update(dt)
	...
	if self.unit then
		...
		self.last_pos = {x=v.x, y=v.y, z=v.z}
	end
	...
	if self.options.draw_skeleton then self:draw_skeleton() end
	if self.options.draw_constraint_targets then self:draw_constraint_targets() end
...
function AnimationEditor:draw_constraint_targets()
	...
	for target,pos in pairs(self.constraint_targets) do
		...
		Gui.text(self.gui, target, "core/editor_slave/gui/arial_df", size, "arial_df",
