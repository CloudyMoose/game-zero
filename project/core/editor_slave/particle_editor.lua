...
function TextDrawer:row(...)
	...
	for i,t in ipairs{...} do
		if i>1 then x = x + self.column_width[i-1] end
...
function ParticleEditor:set_camera_distance(d)
	if d == self.camera_distance then return end
...
function ParticleEditor:update(dt)
	...
	if self.options.show_stats then		self:draw_stats() end
	if self.options.show_ruler then		self:draw_ruler() end
	if self.options.show_variables then	self:draw_variables() end
	if self.flash then					self:draw_flash() end
	
	if self.drag then self:do_drag() end
...
function ParticleEditor:draw_ruler()
	-- Mobile platforms require the bitmap to be power-of-two and rectangular.
	local platform = Application.platform()
	local bitmap_size = (platform == "android" or platform == "ios") and Vector2(2.6,2.6) or Vector2(1.54,2)

	Gui.bitmap(self.ruler_gui, "human", Vector2(0, 0), bitmap_size)
...
function ParticleEditor:draw_variables()
	...
	for i,var in ipairs(self.variables or {}) do
		...
		Gui.text(self.gui, var.name, "core/editor_slave/gui/arial_df", size, "arial_df",
			Vector3(p.x-max.x/2, p.z-max.z/2 - size, 0), text_color)
	end
end
...
function ParticleEditor:mouse_down()
	if self.options.show_variables then
		...
		if target then
			self.drag = {
				variable = target,
				start = {x = self.mouse.x, y = self.mouse.y},
...
function ParticleEditor:drag_pos(start_pos, cam_delta, lock)
	...
	local camera_vector = function(axis) 
	...
	local n = z
	local axis = "xy"
	if size_z > size_x and size_z > size_y then axis = "z" end
	if lock then 
		axis = lock
